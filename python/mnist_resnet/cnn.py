...
class CNN:
    ...
    def layer_conv(self, flow, W_size, b_size):
    ...
    def build(self):
        ...
        # CONV1_1
        flow_ = self.layer_conv(flow, [3,3,1,8], [8])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,8,8], [8])
        flow = self.layer_conv(flow, [1,1,1,8], [8])
        flow += flow_
        flow = self.layer_relu(flow)
        
        # CONV1_2
        flow_ = self.layer_conv(flow, [3,3,8,8], [8])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,8,8], [8])
        flow = self.layer_conv(flow, [1,1,8,8], [8])
        flow += flow_
        flow = self.layer_relu(flow)
        ...
        # CONV2_1
        flow_ = self.layer_conv(flow, [3,3,8,16], [16])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,16,16], [16])
        flow = self.layer_conv(flow, [1,1,8,16], [16])
        flow += flow_
        flow = self.layer_relu(flow)
        
        # CONV2_2
        flow_ = self.layer_conv(flow, [3,3,16,16], [16])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,16,16], [16])
        flow = self.layer_conv(flow, [1,1,16,16], [16])
        flow += flow_
        flow = self.layer_relu(flow)
        ...
        # CONV3_1
        flow_ = self.layer_conv(flow, [3,3,16,32], [32])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,32,32], [32])
        flow = self.layer_conv(flow, [1,1,16,32], [32])
        flow += flow_
        flow = self.layer_relu(flow)
        
        # CONV3_2
        flow_ = self.layer_conv(flow, [3,3,32,32], [32])
        flow_ = self.layer_relu(flow_)
        flow_ = self.layer_conv(flow_, [3,3,32,32], [32])
        flow = self.layer_conv(flow, [1,1,32,32], [32])
        flow += flow_
        flow = self.layer_relu(flow)

        # FC1
        flow = tf.reshape(flow, [-1, 7*7*32])
        flow = self.layer_fc(flow, [7*7*32, 120], [120])
        # Dropout
